...
5. AWS/EKS 비용이 발생할 수 있는 실습은 현재 학습 범위에서 제외하고, 필요한 경우 개념/설계 문서로만 다룬다.
...
- 클라우드 실습은 비용 제약 때문에 실행하지 않는다. AWS/EKS/Karpenter는 개념, 설계, 운영 관점 문서로만 다룬다.
...
9. `terraform apply`, `terragrunt run apply`, `kubectl delete` 기반 cluster-wide resource 삭제, 실제 AWS 리소스 생성 명령은 사용자의 명시적 비용 승인 없이는 실행 대상으로 작성하지 않는다.
