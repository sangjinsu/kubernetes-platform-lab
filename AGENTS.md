...
- 비용 문제로 AWS/EKS/NHN Cloud는 기본 학습 환경에서 사용하지 않는다.
- Karpenter만 예외적으로 명시적 비용 승인 후 AWS EKS 선택 실습으로 다룰 수 있다.
...
- 사용자의 명시적 비용 승인 없이는 `terraform apply`, `terragrunt run apply`, 실제 AWS/NHN Cloud 리소스 생성, cluster-wide 삭제 명령을 실행하지 않는다.
